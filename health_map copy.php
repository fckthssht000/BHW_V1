...
// --- [NEW] HANDLE SETTINGS UPDATE (POST REQUEST) ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_settings'])) {
    $metric_key = $_POST['metric_key'];
    $low = $_POST['low_threshold'];
    $high = $_POST['high_threshold'];
    $c_low = $_POST['color_low'];
    $c_high = $_POST['color_high'];
    // Assuming medium color is calculated or fixed, or passed if you add input for it
    $c_med = $_POST['color_medium'] ?? '#ffc107'; 
    
    // Update specific setting for this user
    $sql = "UPDATE custom_map_settings SET 
            low_threshold=?, high_threshold=?, color_low=?, color_medium=?, color_high=? 
            WHERE user_id=? AND metric_key=?";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$low, $high, $c_low, $c_med, $c_high, $_SESSION['user_id'], $metric_key]);
    
    header("Location: health_map.php"); // Refresh to apply
    exit;
}
...
// Fallback to Admin (ID 1) if user has no settings
if(empty($rows)) {
...
if (!empty($rows)) {
    ...
    // Hardcoded fallback to prevent crash if DB is empty
    $defaults = [
        'underweight_rate' => ['metric_label'=>'Underweight Rate', 'low_threshold'=>5, 'high_threshold'=>10, 'color_low'=>'#28a745', 'color_medium'=>'#ffc107', 'color_high'=>'#dc3545', 'is_higher_better'=>0],
        'stunted_rate' => ['metric_label'=>'Stunted Rate', 'low_threshold'=>10, 'high_threshold'=>20, 'color_low'=>'#28a745', 'color_medium'=>'#ffc107', 'color_high'=>'#dc3545', 'is_higher_better'=>0],
        'immunization_coverage' => ['metric_label'=>'Immunization Coverage', 'low_threshold'=>80, 'high_threshold'=>95, 'color_low'=>'#dc3545', 'color_medium'=>'#ffc107', 'color_high'=>'#28a745', 'is_higher_better'=>1],
        'hypertensive_rate' => ['metric_label'=>'Hypertension Rate', 'low_threshold'=>5, 'high_threshold'=>15, 'color_low'=>'#28a745', 'color_medium'=>'#ffc107', 'color_high'=>'#dc3545', 'is_higher_better'=>0],
    ];
...
<body>
    ...
    <script>
        ...
        function getColor(value, metric) {
            // For coverage/positive metrics (higher is better)
            ...
            if (metric.includes('rate') || metric === 'health_score') {
        ...
        function updateLegend(metric) {
            ...
            var higherIsBetter = metric === 'immunization_coverage' || metric === 'prenatal_coverage' || 
                                 metric === 'exclusive_breastfeeding_rate' || metric === 'fp_intent_rate';
            
            if (higherIsBetter) {
                labels = [
                    { color: '#d73027', label: '0-30%' },
                    { color: '#fc8d59', label: '31-50%' },
                    { color: '#fee08b', label: '51-70%' },
                    { color: '#91cf60', label: '71-90%' },
                    { color: '#1a9850', label: '90%+' }
                ];
            } else if (metric === 'total_population') {
                labels = [
                ...
                labels = [
                    { color: '#1a9850', label: '0-5%' },
                    { color: '#91cf60', label: '6-15%' },
                    { color: '#fee08b', label: '16-30%' },
                    { color: '#fc8d59', label: '31-50%' },
        ...
        document.getElementById('heatmap-metric').addEventListener('change', function() {
            ...
            updateMetricSummary(this.value, document.getElementById('purok-filter').value);
        });
        ...
        purokFilter.addEventListener('change', function() {
            updateHouseholdMarkers();
            updateMetricSummary(document.getElementById('heatmap-metric').value, this.value);
        });
