...
$type = isset($_GET['type']) && $_GET['type'] === 'manual' ? 'manual' : 'automatic';
$backup_dir = "backup/$type/";
...
// Base directory (BHWs have subfolder by purok)
...
try {
    foreach ($record_types as $file => $record_type) {
        ...
        // Senior medication (unchanged logic, still uses junction table senior_medication)
        ...
        // Infant records – updated to use child_immunization + immunization junction
        ...
        // Postnatal records – updated to use pregnancy_medication junction
        ...
        // Child health records (unchanged)
        ...
        // Pregnant records – updated to use pregnancy_medication junction and pregnancy_period filter
    ...
    // For BHW, also mirror into backup/[type]/all/
    if ($role_id == 2 && $user_purok) {
    ...
    $_SESSION['backup_success'] = "Backup created successfully in $base_dir";
    header("Location: settings.php");
    exit;
} catch (PDOException $e) {
    $_SESSION['backup_error'] = "Backup failed: " . $e->getMessage();
    file_put_contents('backup_errors.log', date('Y-m-d H:i:s') . " - Backup failed: " . $e->getMessage() . "\n", FILE_APPEND);
    header("Location: settings.php");
    exit;
}
?>
