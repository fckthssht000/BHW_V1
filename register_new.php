...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
    ...
    // Validate name fields (allow letters, spaces, commas, dots, hyphens)
    if (!empty($last_name) && !preg_match('/^[a-zA-Z\s,.\-]*$/', $last_name)) {
        $errors[] = "Last name can only contain letters, spaces, dots, commas, and hyphens.";
    }
    if (!empty($first_name) && !preg_match('/^[a-zA-Z\s,.\-]*$/', $first_name)) {
        $errors[] = "First name can only contain letters, spaces, dots, commas, and hyphens.";
    }
    if (!empty($middle_initial) && !preg_match('/^[a-zA-Z\s,.\-]*$/', $middle_initial)) {
        $errors[] = "Middle initial can only contain letters, spaces, dots, commas, and hyphens.";
    }
...
// Handle AJAX requests for household number
...
<body>
    ...
    <script>
        ...
        function capitalizeWords(input) {
            return input.replace(/(^|\s|[-,.\-])([a-z])/g, (match, sep, letter) => sep + letter.toUpperCase());
        }
